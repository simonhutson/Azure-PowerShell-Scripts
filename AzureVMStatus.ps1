...
foreach ($Subscription in $Subscriptions)
{
    ...
    if($VMs)
    {
        ...
        foreach ($VM in $VMs)
        {
            ...
            $VMObject | Add-Member -MemberType NoteProperty -Name "Windows Hybrid Benefit" -Value $(if((Get-ChildObject -Object $VM -Path Properties.StorageProfile.OsDisk.OsType) -eq "Windows"){if(Get-ChildObject -Object $VM -Path LicenseType){"Enabled"}else{"Not Enabled"}}else{"Not Supported"})
