###############################################################################################################################
#
# Retrieve the Subscription usages by region, associated with a specific Azure AD Tenant
#
...
foreach($Subscription in $SelectedSubscriptions)
{
    ...
    $Context = Set-AzureRmContext -SubscriptionId $Subscription.Id -TenantId $Tenant.Id
    Write-Host
    ...
    {
        Get-AzureRmVMUsage -Location $Location.Location
    }
}
